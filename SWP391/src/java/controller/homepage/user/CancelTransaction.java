...
public class CancelTransaction extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CancelTransaction at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        boolean check = dao.cancelAppointment(appointment_id, "Canceled");
        if(check==true){
            session.setAttribute("ms", "Giao dịch quá hạn đã bị hủy.");
