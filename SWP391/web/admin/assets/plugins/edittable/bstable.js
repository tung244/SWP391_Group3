...
class BSTable {
    ...
    constructor(tableId, options) {
        ...
        var defaults = {
            ...
            advanced: {// Do not override advanced unless you know what youre doing
                ...
                <button id="bView" type="button" class="btn btn-sm btn-default">
    ...
    _rowEdit(button) {
        ...
        console.log($cols);
        if (this.currentlyEditingRow($row))
            return;    // not currently editing, return
    ...
    _rowView(button) {
        var $row = $(button).parents('tr'); // Lấy dòng
        var $cols = $row.find('td');         // Lấy các ô trong dòng
        var content = '';

        // Lấy dữ liệu từ các ô và thêm vào bảng
        $cols.each(function (index) {
            var header = $(this).data('header'); // Giả sử bạn có thuộc tính data-header trên các ô
            content += '<tr>';
            content += '<td>' + header + '</td>'; // Hiển thị tiêu đề (hoặc tên cột)
            content += '<td>' + $(this).text() + '</td>'; // Hiển thị giá trị
            content += '</tr>';
        });
    ...
    _modifyEachColumn($editableColumns, $cols, howToModify) {
        ...
        $cols.each(function () {
            ...
            if ($(this).attr('name') == 'bstable-actions')
                return;    // exclude the actions column
            if (!isEditableColumn(n - 1))
                return;                     // Check if the column is editable
        ...
        function isEditableColumn(columnIndex) {
            ...
            if ($editableColumns == null) {                           // option not defined
                ...
                for (var i = 0; i < $editableColumns.length; i++) {
                    if (columnIndex == $editableColumns[i])
                        return true;
                }
    ...
    _addOnClickEventsToActions() {
        ...
        this.table.find('tbody tr #bView').each(function () {
            let button = this;
            button.onclick = function () {
                _this._rowView(button); // Gọi phương thức _rowView khi nút "View" được nhấn
            };
        });
